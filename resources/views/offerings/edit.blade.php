...
@section('title', 'Update Offering')

@section('content')
    <div class="row">
        <div class="col-xl">
            <form method="POST" action="{{ route('offerings.update', ['offering' => $offering]) }}"
                    ...
                    <div class="card-body">
                            ...
                            <select name="establishment" id="select_mode" class="select_mode form-select">
                                ...
                                @foreach ($establishments as $establishment)
                                    <option value="{{ $establishment->id }}"
                                        @if ($establishment->id == $offering->establishment_id) selected @endif>{{ $establishment->name }}
                                    </option>
                                @endforeach
                            </select>
                            @error('establishment')
                        ...
                        <div class="mb-6">
                            ...
                            <input type="file" name="image" id="image" class="form-control"
                                value="{{ old('image', $offering->path) }}" accept="image/*">
                            @error('image')
                                <small class="text-danger">{{ $message }}</small>
                            @enderror
                        </div>
                        ...
                        <div class="mb-6">
                            ...
                            <textarea id="description" name="description" class="form-control" rows="30">{{ old('description', $offering->description) }}</textarea>
                            @error('description')
                                <small class="text-danger">{{ $message }}</small>
                            @enderror
                        </div>
